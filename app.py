...
app = Flask(__name__, static_folder='fintechlendingapp/build', static_url_path='')
origins = [
    "http://localhost:3000",
    "http://localhost:3001",  # Allow your React frontend origin
    "http://localhost:3002"
...
#the below commented code is used for trining the model, as it taking too much memory
#so its better to do the training the model once and save the model to use it later
...
# dump(clasify, 'knn_model.joblib')
# dump(enc, 'encoder.joblib')
# dump(scaler, 'scaler.joblib')
# dump(x_help, 'x_help.joblib')
# dump(y_pred, 'y_pred.joblib')
# dump(df, 'df.joblib')

# Load the models at the start
clasify = load('knn_model.joblib')
enc = load('encoder.joblib')
scaler = load('scaler.joblib')
x_help = load('x_help.joblib')
y_pred = load('y_pred.joblib')
df = load('df.joblib')
...
def run(inp):
    ...
    if value==0:
        ...
        # return jsonify(value)
        return value,"Congratulations! Your loan approved!!",scored
#stuff ---------------------------------
...
def index():
    ...
    return{
        "value": int(result[0]),
        "nearestneighbour": str(result[1]),
        "creditgrade": str(result[2]),
    }
